...
async function showPage(page_no) {
    ...
    convertPageTotiff(page, page_no).then(tiffDataUrl => {
        _tiff_STORE[page_no] = tiffDataUrl;
    });
}


async function convertPageTotiff(page_no) {
    try {
        const page = await _PDF_DOC.getPage(page_no);
        const scale_factor = 6; // Increase this value for better quality of tiff
        const dpi = 1200; // Increase this value for better quality of tiff
...
async function downloadZip() {
    ...
    for (let page_no = 1; page_no <= _TOTAL_PAGES; page_no++) {
        ...
        const tiffDataUrl = await convertPageTotiff(page_no);
        window.zip.file(`page-${page_no}.tiff`, tiffDataUrl.split(';base64,').pop(), { base64: true });
    }
